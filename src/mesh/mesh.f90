...
module class_mesh
  ...
  type, public, extends(named) :: mesh
...
contains
  ...
  subroutine init(m)
    ...
    if( .not. associated(m%x) .and. m % nx > 0 ) then
       allocate( m % x ( m % nx ) )
       allocate( m % dx( m % nx ) )
       call m%log(FPDE_LOG_DEBUG, "init: generating from nx")
    else
       call m%log(FPDE_LOG_ERROR,&
            "init: neither, x(:) nor nx were set correctly")
    end if
  ...
  subroutine generate_points(m)
    ...
    if( associated( m % x ) .and. size(m%x) /= m%nx ) then
       ...
       if( m % x(m%nx) < m % x(1) ) then
          call m%log(FPDE_LOG_INFO, "generate_x: x(:) runs backward")
       end if

    else
       call m%log(FPDE_LOG_ERROR, "generate_points: x(:) is not associated")
    end if
  ...
  function diff_point( m, f, k, i ) result(d)
    ...
    call m%log(FPDE_LOG_ERROR, &
         "ERROR: mesh: diff_point: subroutine not overloaded")
  ...
  subroutine diff_global( m, f, df, k )
    ...
    call m%log(FPDE_LOG_ERROR,&
         "internal_global_d: called with storage_active = .false.")
